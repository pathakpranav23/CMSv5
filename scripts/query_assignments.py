...
from cms_app.models import Faculty, CourseAssignment, Subject, Division, User
...
def find_faculty_by_name(name_query: str):
    ...
    q = db.session.execute(
        select(Faculty).where(Faculty.full_name.ilike(f"%{name_query}%"))
    ).scalars().all()
    if q:
        return q[0]
    name_trim = name_query.replace("Dr ", "").strip()
    q = db.session.execute(
        select(Faculty).where(Faculty.full_name.ilike(f"%{name_trim}%"))
    ).scalars().all()
...
def find_faculty_by_email(email_query: str):
    ...
    fac = db.session.execute(
        select(Faculty).where(Faculty.email.ilike(email_query))
    ).scalars().first()
    if fac:
        return fac
    # Next, try linked User by username (often email)
    user = db.session.execute(
        select(User).where(User.username.ilike(email_query))
    ).scalars().first()
    if user:
        fac = db.session.execute(
...
def main():
    ...
    with app.app_context():
        ...
        assignments = db.session.execute(
            select(CourseAssignment).filter_by(faculty_id_fk=user.user_id, is_active=True)
        ).scalars().all()
        if not assignments:
        ...
        for a in assignments:
            subj = db.session.get(Subject, a.subject_id_fk)
            div = db.session.get(Division, a.division_id_fk) if a.division_id_fk else None
            results.append({
